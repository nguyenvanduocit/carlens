...
// Type for valid metric keys
export type CsvMetricKey = keyof typeof CSV_METRICS;

// Type for valid metric values
...
export function isKnownMetric(header: string): header is CsvMetricValue {
  return AVAILABLE_METRICS.includes(header as CsvMetricValue);
}

// Helper function to generate constant names for new metrics
export function generateMetricConstantName(metricValue: string): string {
  return metricValue
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, '_')
    .replace(/_+/g, '_')
